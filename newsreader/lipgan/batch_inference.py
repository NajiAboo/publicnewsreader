...
import numpy as np
import scipy, cv2, os, sys, argparse, lipgan.audio as audio
...
# parser.add_argument('--results_filename', type=str, help='Folder to save all results into', default='results.mp4')
# parser.add_argument('--results_voice_filename', type=str, help='Folder to save all results into', default='results_voice.mp4')
...
from lipgan.generator import create_model_residual as create_model
#else:
...
def main(args1):
	...
	for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, 
		...
		if i == 0:
			...
			out = cv2.VideoWriter(path.join(args.results_dir, args.results_filename), 
	...
	command = 'ffmpeg -i {} -i {} -strict -2 -q:v 1 {}'.format(args.audio, path.join(args.results_dir, args.results_filename), 
														path.join(args.results_dir, args.results_voice_filename))
